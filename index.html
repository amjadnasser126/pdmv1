...
const INIT_ELIG = {
  PTV_CDI: { SNT:1,TRT:1,VAC:1,AEM:1,AMH:1,CCH:1,NCD:1,MAV:1,OPS:1,GAM:1,MUL:1,AST:1,AMD:1 },
  PTV_CDD: { SNT:1,TRT:1,VAC:1,AEM:1,AMH:1,CCH:1,NCD:1,MAV:1,OPS:1,GAM:1,MUL:1,AST:0,AMD:0 },
  PTV_ALT: { SNT:1,TRT:1,VAC:1,AEM:1,AMH:1,CCH:1,NCD:1,MAV:1,OPS:1,GAM:1,MUL:1,AST:0,AMD:0 },
  PTM_CDI: { SNT:0,TRT:0,VAC:0,AEM:0,AMH:0,CCH:0,NCD:0,MAV:0,OPS:0,GAM:0,MUL:0,AST:0,AMD:0 },
  PTM_CDD: { SNT:1,TRT:1,VAC:0,AEM:0,AMH:0,CCH:0,NCD:1,MAV:1,OPS:0,GAM:0,MUL:0,AST:0,AMD:0 },
  PTM_ALT: { SNT:0,TRT:0,VAC:0,AEM:0,AMH:0,CCH:0,NCD:0,MAV:0,OPS:0,GAM:0,MUL:0,AST:0,AMD:0 },
  PTS_CDI: { SNT:1,TRT:1,VAC:0,AEM:1,AMH:1,CCH:0,NCD:1,MAV:1,OPS:0,GAM:0,MUL:0,AST:0,AMD:0 },
  PTS_CDD: { SNT:1,TRT:1,VAC:0,AEM:0,AMH:0,CCH:0,NCD:1,MAV:1,OPS:0,GAM:0,MUL:0,AST:0,AMD:0 },
  PTS_ALT: { SNT:1,TRT:1,VAC:0,AEM:0,AMH:0,CCH:0,NCD:1,MAV:1,OPS:0,GAM:0,MUL:0,AST:0,AMD:0 },
};
...
function Dashboard() {
  ...
  const { synthese12, detailMois } = useMemo(() => {
    ...
    // Capacite ALLOUEE par PE = repartition proportionnelle (au prorata du besoin) de chaque case
    // site_statut cochee entre tous les PE eligibles a cette case. Contrairement a une simple somme
    // des cases cochees (qui compte plusieurs fois la meme capacite si plusieurs PE partagent la case),
    // cette allocation garantit que la somme des "Capacite allouee" de tous les PE = capacite nationale
    // totale, et que la somme des Deltas par PE = Delta national exact (plus de contradiction visuelle).
    const capaciteAlloueeParPE = {};
    PE_LIST.forEach(({ key }) => { capaciteAlloueeParPE[key] = 0; });
    SITE_STATUTS.forEach((ss) => {
      const eligibles = PE_LIST.filter(({ key }) => elig[ss][key] === 1).map((p) => p.key);
      const poolCell = poolSiteStatut[ss] || 0;
      if (eligibles.length === 0 || poolCell === 0) return;
      const totalBesoinE = eligibles.reduce((a, k) => a + besoinHeures[k], 0);
      if (totalBesoinE > 0) {
        eligibles.forEach((k) => { capaciteAlloueeParPE[k] += poolCell * (besoinHeures[k] / totalBesoinE); });
      } else {
        // aucun besoin ce mois sur les PE eligibles a cette case -> capacite repartie a parts egales (inutilisee)
        eligibles.forEach((k) => { capaciteAlloueeParPE[k] += poolCell / eligibles.length; });
      }
    });
    const typeParPE = {};
    PE_LIST.forEach(({ key }) => {
    ...
    return {
      synthese12: synth,
      detailMois: {
        poolSite, poolSiteStatut, capaciteAlloueeParPE, sitesEligiblesParPE, besoinHeures, typeParPE, besoinMonoParSite,
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 text-sm space-y-6">
      <div>
        <h2 className="text-lg font-medium mb-1">Dashboard PDM — matrices mensuelles éditables (13 PE)</h2>
        <p className="text-gray-500 text-xs">Effectifs, heures/personne, présence, productivité et appels prévus sont éditables mois par mois. Tout se recalcule en direct et reste relié : Effectifs → Capacité, Appels prévus ÷ Productivité → Besoin, Compétences → Capacité allouée par PE.</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-gray-50 rounded-lg p-3">
          <p className="text-xs text-gray-500">Besoin (13 PE) — {MOIS[moisIdx]}</p>
      ...
      {ongletActif === 'resultats' && (
        <div>
          <p className="text-xs text-gray-400 mb-2">
            "Capacité allouée" = part de chaque case cochée dans l'onglet Compétences attribuée à ce PE, au prorata de son besoin par rapport aux autres PE éligibles à la même case (donc plus de double-comptage entre PE). La somme des Deltas par PE ci-dessous égale le Delta national, sauf capacité non attribuable à aucun des 13 PE (voir alerte ci-dessous si applicable). "Besoin (h)" = Appels prévus ÷ Productivité.
          </p>
          {(() => {
            const sommeCapaciteAllouee = PE_LIST.reduce((a, { key }) => a + detailMois.capaciteAlloueeParPE[key], 0);
            const capaciteNonAllouable = detailMois.capaciteTotale - sommeCapaciteAllouee;
            const sommeDeltaPE = PE_LIST.reduce((a, { key }) => a + (detailMois.capaciteAlloueeParPE[key] - detailMois.besoinHeures[key]), 0);
            return capaciteNonAllouable > 0.5 ? (
              <p className="text-xs text-amber-700 bg-amber-50 rounded px-2 py-1 mb-2">
                ⚠️ {fmt(capaciteNonAllouable)}h de capacité ({MOIS[moisIdx]}) ne sont éligibles à aucun des 13 PE suivis (probablement dédiées à un PE hors périmètre, ex. TLV EPI) — non comptées dans la somme des Deltas par PE ci-dessous ({fmt(sommeDeltaPE)}h), qui diffère donc du Delta national affiché plus haut ({fmt(detailMois.deltaNational)}h) exactement de ce montant.
              </p>
            ) : null;
          })()}
          <table className="border-collapse w-full">
            <thead>
              <tr className="text-gray-500">
                ...
                <th className="pb-1 pr-2 font-normal">Capacité allouée (h)</th>
            ...
            <tbody>
              {PE_LIST.map(({ key, name }) => {
                const deltaInd = detailMois.capaciteAlloueeParPE[key] - detailMois.besoinHeures[key];
                return (
                  <tr key={key} className="border-t border-gray-100">
                    ...
                    <td className="py-1 pr-2 text-right text-blue-700">{fmt(detailMois.capaciteAlloueeParPE[key])}</td>
      ...
      {ongletActif === 'competences' && (
        <div className="overflow-x-auto">
          <p className="text-xs text-gray-400 mb-2">
            Ne varie pas dans le temps (simplification assumée). Formule reliée : Capacité allouée[pe] = répartition proportionnelle au besoin de chaque case cochée — la somme sur tous les PE égale la capacité nationale, aucun double-comptage.
          ...
          <table className="border-collapse text-xs">
            ...
            <tbody>
              ...
              <tr className="border-t-2 border-gray-400 bg-blue-50">
                <td className="pr-2 py-1 font-medium sticky left-0 bg-blue-50">Capacité allouée (h) — {MOIS[moisIdx]}</td>
                {PE_LIST.map(({ key }) => (
                  <td key={key} className="text-center px-1 py-1 font-medium text-blue-700">{fmt(detailMois.capaciteAlloueeParPE[key])}</td>
                ))}
              </tr>
              ...
              <tr>
                ...
                {PE_LIST.map(({ key }) => {
                  const d = detailMois.capaciteAlloueeParPE[key] - detailMois.besoinHeures[key];
      ...
      <p className="text-xs text-gray-400">
        Rappel : 13 PE sur 17 couverts (TLVINTGG, TLV EPI, TLV SPE2, SPE2 manquants). Besoin monosite = somme
